...
void lsa_init() {
    ...
    for (uint8_t port = 0; port < node_config.num_neighbors; port++)
        graph_add_edge(g, node_config.node_addr, neighbor_addrs[port], node_config.link_costs[port]);
}
...
int lsa_update(mixnet_packet_lsa *lsa_packet) {
    ...
    if (g->open_edges == 0) {
        ...
        ll_append(shortest_paths[node_config.node_addr]->route, node_config.node_addr);
        ...
        while (!pq_empty(pq)) {
            ...
            for (int i = 0; i < added->n_neighbors; i++) {
